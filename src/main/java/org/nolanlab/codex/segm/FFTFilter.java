 ...
 * "http://imagej.nih.gov/ij/plugins/fft-filter.html". 2001/10/29: First Version
 * (JW) 2003/02/06: 1st bugfix (works in macros/plugins, works on stacks,
 * overwrites image(=>filter)) (JW) 2003/07/03: integrated into ImageJ, added
 * "Display Filter" option (WSR) 2007/03/26: 2nd bugfix (Fixed incorrect
 * calculation of filter from structure sizes, which caused the real structure
 * sizes to be wrong by a factor of 0.75 to 1.5 depending on the image size.)
 */
public class FFTFilter {
    ...
    public void run(ImagePlus imp) {
        ExecutorService es = java.util.concurrent.Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors());
        boolean isStack = imp.getStackSize() > 1;
        this.imp = imp;
        if (!isStack) {
            filter(imp.getProcessor());
        } else {
            ...
            for (int i = 1; i <= imp.getNSlices(); i++) {
                ...
                es.submit(new Runnable() {
                    public void run() {
                        System.out.println("Processing slice #" + idx);
                        filter(is.getProcessor(idx));
                    }
                });
            }
            try {
    ...
    public ImageProcessor tileMirror(ImageProcessor ip, int width, int height, int x, int y) {
        if (IJ.debugMode) {
            IJ.log("FFT.tileMirror: " + width + "x" + height + " " + ip);
        }
    ...
    public void setup(double filterLarge, double filterSmall, int supressStripes, double toleranceDir, boolean autoScale, boolean saturate, boolean displayFilter) {
        FFTFilter.filterLargeDia = filterLarge;
        FFTFilter.filterSmallDia = filterSmall;
        FFTFilter.choiceIndex = supressStripes;
        FFTFilter.choiceDia = choices[choiceIndex];
        FFTFilter.toleranceDia = toleranceDir;
        FFTFilter.doScalingDia = autoScale;
        FFTFilter.saturateDia = saturate;
        FFTFilter.displayFilter = displayFilter;
    }
