...
/**
 * The Stack_Normalizer recalculates the grey levels of the stack, so that the
 * minimum and maximum grey level after normalization are equal to the specified
 * values. The minimum and maximum grey levels are determined in the whole stack
 * and not just in one plane. For RGB images all channels are normalized to the
 * same min/max values.
 */
public class StackNormalizer {
    ...
    public void normalize(ImagePlus imp, int min, int max) {
        newMin = min;
        newMax = max;
